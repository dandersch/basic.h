...
clang++ -g -Wno-deprecated -std=c++11 test.c test2.c -o test_clangxx && ./test_clangxx

echo ""
echo "Testing g++, c++20:"
rm test_gxx 2>/dev/null
g++ -g test.c test2.c -std=c++20 -o test_gxx && ./test_gxx

echo ""
echo "Testing clang, c11:"
rm test_clang 2>/dev/null
clang -g -std=c11 test.c test2.c -o test_clang && ./test_clang

echo ""
echo "Testing gcc, c99, 32bit:"
rm test_gcc 2>/dev/null
gcc -g -m32 -DBUILD_DEBUG -std=c99 -O2 test.c test2.c -o test_gcc && ./test_gcc

echo ""
echo "Testing mingw-g++:"
rm test_mingwxx.exe 2>/dev/null
x86_64-w64-mingw32-g++ -g test.c test2.c -o test_mingwxx && WINEDEBUG=-all wine ./test_mingwxx.exe

echo ""
echo "Testing mingw-gcc:"
rm test_mingw.exe 2>/dev/null
x86_64-w64-mingw32-gcc -g test.c test2.c -o test_mingw && WINEDEBUG=-all wine ./test_mingw.exe
...
cl.exe /std:c++14 test.cpp test2.cpp /link /OUT:test_msvcxx.exe /SUBSYSTEM:CONSOLE && WINEDEBUG=-all wine ./test_msvcxx.exe
...
cl.exe /std:c17 test.c test2.c /link /OUT:test_msvc.exe /SUBSYSTEM:CONSOLE && WINEDEBUG=-all wine ./test_msvc.exe
